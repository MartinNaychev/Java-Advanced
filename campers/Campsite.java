...
public class Campsite {
    ...
    public String getMostExperiencedCamper() {
        ...
        if (campers.isEmpty()) {
            return String.format("There are no campers at %s.", this.name);
        } else {
    ...
    public String getCampersWithPersonalTents() {
        ...
        if (!hasACamperWithTent) {
            stringBuilder.append(String.format("No campers currently at %s.", this.name));
        } else {
    ...
    public String getReport() {
        ...
        int counter = 1;
        if (!campers.isEmpty()) {
            stringBuilder.append(String.format("Campsite %s hosts the following campers:", this.name));
            for (Camper camper : campers) {
            ...
            stringBuilder.append(String.format("There are no campers at %s.", this.name));
        }
